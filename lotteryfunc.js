...
//function that gives win categories
function createWins(winSpecs) {
    const winCategories;
...
//price and winnings
function priceAnalytics(firstanalysis, gamePrice) {
    const { comparison, matchCount } = firstanalysis;
    const jackpotPrice = (comparison.length  * gamePrice)/2;
    //win categories for all price
    const winCategories = {
        5: jackpotPrice * (50/100),
        4: jackpotPrice * (30/100),
        3: jackpotPrice * (15/100),
        2: jackpotPrice * (5/100),
        1: 0,
        0: 0,
    }
    let allWinnings = {};
